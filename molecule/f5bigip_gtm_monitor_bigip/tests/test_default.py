...
def test_default(host):
    ...
    monitor = mgmt.tm.gtm.monitor.bigips.bigip.load(
        name="my_bigip_monitor", partition="Common"
    )
    assert monitor.name == "my_bigip_monitor"
    assert monitor.partition == "Common"
    assert monitor.description == "My bigip monitor"
