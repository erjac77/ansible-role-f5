...
def test_default(host):
    ...
    monitor = mgmt.tm.gtm.monitor.gateway_icmps.gateway_icmp.load(
        name="my_gateway_icmp_monitor", partition="Common"
    )
    assert monitor.name == "my_gateway_icmp_monitor"
    assert monitor.partition == "Common"
    assert monitor.description == "My gateway icmp monitor"
