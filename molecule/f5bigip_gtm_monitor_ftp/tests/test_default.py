...
def test_default(host):
    ...
    monitor = mgmt.tm.gtm.monitor.ftps.ftp.load(
        name="my_ftp_monitor", partition="Common"
    )
    assert monitor.name == "my_ftp_monitor"
    assert monitor.partition == "Common"
    assert monitor.description == "My ftp monitor"
