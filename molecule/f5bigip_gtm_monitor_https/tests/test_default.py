...
def test_default(host):
    ...
    monitor = mgmt.tm.gtm.monitor.https_s.https.load(
        name="my_https_monitor", partition="Common"
    )
    assert monitor.name == "my_https_monitor"
    assert monitor.partition == "Common"
    assert monitor.description == "My https monitor"
